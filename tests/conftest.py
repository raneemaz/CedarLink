...
Foreign keys are enforced in **every** configuration since ADR 0033 — the
``Engine.connect`` listener that issues ``PRAGMA foreign_keys=ON`` lives in
``app/extensions.py`` now, not here. See
